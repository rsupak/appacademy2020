...
def chain_map(value, procs)
  ...
  # value
  procs.inject(value) { |acc, prc| prc.call(acc) }
end
...
def proctition_platinum(arr, *procs)
  prc_hash = {}
  procs.each_index { |i| prc_hash[i + 1] = [] }
  arr.each do |el|
    procs.each.with_index do |prc, i|
      if prc.call(el)
        prc_hash[i + 1] << el
        break
      end
...
def procipher(str, hash)
  ...
  hash.each do |prc1, prc2|
    str.each.with_index do |word, i|
      str[i] = prc2.call(word) if prc1.call(original[i])
    end
  end
  str.join(' ')
end
